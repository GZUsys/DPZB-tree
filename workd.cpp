...
int main()
{
	...
	sprintf(loading_file, "%s", "/root/lmj/datafile/workloadd.csv");
	...
	for (int i = 0; i < test_num; i++) {
		...
		if (buf.size() == bench_size) {
			clock_gettime(CLOCK_MONOTONIC, &T1);
			std::vector<uint64_t> hot_key = KMeans(buf, 1, 2);
			for (const auto& key : hot_key) {
				T.hot_node_dispose(key);
			}
			clock_gettime(CLOCK_MONOTONIC, &T2);
			latency1 = ((T2.tv_sec - T1.tv_sec) * 1000000000 + (T2.tv_nsec - T1.tv_nsec)) / 100;
